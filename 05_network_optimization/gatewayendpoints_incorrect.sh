...
do
    ...
    do
        ...
        route_exists=$(aws ec2 describe-route-tables --region $region --filters Name=vpc-id,Values=$vpc Name=route.destination-cidr-block,Values=0.0.0.0/0 --query 'RouteTables[*].Routes[*].GatewayId' --output text)
        if [ -z "$route_exists" ]
        then
            # If the route does not exist, create it
            aws ec2 create-route --route-table-id $(aws ec2 describe-route-tables --region $region --filters Name=vpc-id,Values=$vpc --query 'RouteTables[*].RouteTableId' --output text) --destination-cidr-block 0.0.0.0/0 --gateway-id $endpoint_id --region $region
